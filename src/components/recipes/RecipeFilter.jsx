...
export default function RecipeFilter({ allRecipes, categories, cuisines, diets }) {
  ...
  const baseFilters = useMemo(() => {
    ...
    return timeFiltered.filter(recipe => {
  ...
  }, [allRecipes, timeFilter, cookwareFilter]);

  const calculateDynamicCounts = (filterType) => {
    return useMemo(() => {
      let filtered = baseFilters;
      if (filterType !== 'category' && activeCategories.size > 0) {
        filtered = filtered.filter(r => activeCategories.has(r.category));
      }
      if (filterType !== 'cuisine' && activeCuisines.size > 0) {
        filtered = filtered.filter(r => r.cuisines?.some(c => activeCuisines.has(c)));
      }
      if (filterType !== 'diet' && activeDiets.size > 0) {
        filtered = filtered.filter(r => r.diets?.some(d => activeDiets.has(d)));
      }
      ...
      return counts;
    }, [baseFilters, activeCategories, activeCuisines, activeDiets]);
  };
      ...
      <div className="space-y-4 mb-12 p-4 border rounded-lg bg-white">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {manuallySortedCategories.map(({ name }) => {
                const count = dynamicCategoryCounts.get(name) || 0;
                return (
                    <button key={name} onClick={() => handleTagToggle(activeCategories, setActiveCategories, name)} className={`sort-btn ${activeCategories.has(name) ? 'active' : ''}`} disabled={!activeCategories.has(name) && count === 0}>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {cuisines.map(({ name }) => {
                const count = dynamicCuisineCounts.get(name) || 0;
                return (
                    <button key={name} onClick={() => handleTagToggle(activeCuisines, setActiveCuisines, name)} className={`sort-btn ${activeCuisines.has(name) ? 'active' : ''}`} disabled={!activeCuisines.has(name) && count === 0}>
                        {name} <span className="ml-1.5 opacity-75">({count})</span>
                    </button>
                );
            })}
         ...
         <div>
          ...
          <div className="flex flex-wrap gap-2">
            {diets.map(({ name }) => {
                const count = dynamicDietCounts.get(name) || 0;
                return (
                    <button key={name} onClick={() => handleTagToggle(activeDiets, setActiveDiets, name)} className={`sort-btn ${activeDiets.has(name) ? 'active' : ''}`} disabled={!activeDiets.has(name) && count === 0}>
                        {name} <span className="ml-1.5 opacity-75">({count})</span>
                    </button>
                );
            })}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {TIME_RANGES.map(range => (
                <button key={range.label} onClick={() => setTimeFilter(range)} className={`sort-btn ${timeFilter.label === range.label ? 'active' : ''}`}>{range.label}</button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {COOKWARE_RANGES.map(range => (
                <button key={range.label} onClick={() => setCookwareFilter(range)} className={`sort-btn ${cookwareFilter.label === range.label ? 'active' : ''}`}>{range.label}</button>
              ))}
            </div>
          </div>
        </div>
